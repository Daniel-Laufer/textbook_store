...
function CompactItemCard({
  ...
  useEffect(() => {
    if (cart.refreshRequested && user.loggedIn && !cart.pending) {
      console.log("CompactCard getCartItems")
  ...
  if (item) {
    console.log("Dark: ", settings.settings.darkTheme)
    return (
      <div className="compact-card" style={{"color": !settings.settings.darkTheme ? 'rgb(0,0,0)': "rgb(255,255,255)"}}>
        ...
        <div className="compact-seller-info-container">
          ...
          <div className="compact-seller-userName">{item.sellerPublicInfo.userName}</div>
        </div>
        <div className="divider"> </div>
        <div className="compact-campus">
          <div><strong>{item.course.toUpperCase()}</strong> at <strong>{item.campus}</strong></div>
        </div>
        <div className="divider"> </div>
        ...
        <div className="compact-card-button-holder">
          <button
            ...
            type="button"
            disabled={!user.loggedIn || displayDelete ? true : false}
          ...
          <button
            ...
            disabled={user.loggedIn && displayDelete ? false : true}
            onClick={() => {
              handleDelete();
            }}
